<script lang="ts">
	...
	$: title = post && ($locale === 'en' ? post.title : post.title_zh);
</script>

<svelte:head>
	<title>{title || 'Article'} - Cesar Cai</title>
</svelte:head>
...
<div class="max-w-3xl mx-auto px-6 py-20">
	...
	{:else if !data.configured}
		<!-- Not configured -->
		<div class="text-center py-20">
			<p class="text-gray-400 mb-4">Supabase not configured</p>
			<a href="/blog" class="text-emerald-400 hover:text-emerald-300">
				← {t('article.back')}
			</a>
		</div>
		...
		<article>
			...
			{:else}
				<div class="text-center py-10">
					<p class="text-gray-400">Content not available</p>
				</div>
			{/if}
		</article>
	{:else}
